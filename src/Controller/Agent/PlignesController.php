...
class PlignesController extends AppController
{
    ...
    public function index()
    {
        ...
        $dossiers = $this->Dossiers->find()->where(['Dossiers.owner_id'=>$client_id])->contain(['Plans'])->all();
       ...
       // debug($dossiers);
        ...
        $plignes = $this->paginate($this->Plignes->find()->where(['pilote_id'=>-1]));
       // debug($plignes);
        if(!empty($plans)){
            $plignes = $this->paginate($this->Plignes->find()->where(['plan_id in'=>$plans,'pilote_id'=>$user_id]));
        }
