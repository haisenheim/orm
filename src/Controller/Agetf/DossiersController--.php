<?php
namespace App\Controller\Agetf;
...
class DossiersController extends AppController
{
    ...
    public function index()
    {
        ...
        if($role==1){
            $this->paginate = [
                'contain' => ['Owners', 'Marketers', 'Autors', 'Produits', 'Timmobilisations', 'Mfinancements']
            ];
            ...
            if($role==2){
                ...
                $dossiers = $this->paginate($this->Dossiers->find()->where(['owner_id in'=>$ids])->contain(['Owners']));
                $clients = $this->Clients->find()->where(['client_id'=>$client])->contain(['Dossiers.Produits','Dossiers.Owners'])->all();
            ...
            if($role==3){
                $this->paginate = [
                    ...
                    =>function($q) use($client){
                        ...
                        'Marketers', 'Autors', 'Produits', 'Timmobilisations', 'Mfinancements']
                ];
                $dossiers = $this->paginate($this->Dossiers->find()->where(['marketer_id'=>$user]));
            }
            ...
            if($role==4){
                ...
                $dossiers = $this->paginate($this->Dossiers->find()->where(['owner_id'=>$user]));
            }
    ...
    public function add($client=null)
    {
        ...
        $owners = $this->Dossiers->Owners->find()->where(['tclient_id'=>3,'client_id'=>$this->Auth->user()['client_id']])->all();
        $marketers = $this->Dossiers->Marketers->find()->where(['client_id'=>$this->Auth->user()['client_id']]);
        ...
        $this->set(compact('dossier', 'owners', 'marketers', 'produits', 'timmobilisations', 'mfinancements','sectors','title'));
    }
