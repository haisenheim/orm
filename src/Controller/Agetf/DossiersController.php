...
class DossiersController extends AppController
{
    ...
    public function index()
    {
        ...
        $dossiers =  $this->paginate($this->Dossiers->find()->where(['Dossiers.client_id'=>$this->Auth->user()['client_id'], 'autor_id'=>$this->Auth->user()['id'],'Dossiers.active'=>1,'archive'=>0])->contain(['Owners']));
    ...
    public function setup($id=null){
        ...
        if($this->request->is('post')){
            $dossier = $this->Dossiers->get($this->request->getData('id'));
            $dossier->commentaire = $this->request->getData('commentaire');
            $dossier->quantity = $this->request->getData('quantity');
            $dossier = $this->Dossiers->save($dossier);
            if($dossier){
                $this->Flash->success('Dossier parametre avec succes !!!');
                return $this->redirect(['action'=>'index']);
            }

            $this->Flash->error('Erreur survenue lors du parametrage du dossier');
        }
    ...
    public function retard($id){
        ...
        if($dossier){
            $this->Flash->success('Mise à jour effectuée avec succès !!!');
        }
    ...
    public function regulariser($id){
        $dossier =$this->Dossiers->get($id);
        $dossier->retard = 0;
        $dossier=$this->Dossiers->save($dossier);

        if($dossier){
            $this->Flash->success('Regularisation effectuée avec succès !!!');
        }
