...
class PlignesController extends AppController
{
    ...
    /*public function index()
    {
        ...
        $dossiers = $this->Dossiers->find()->where(['Dossiers.expert_id'=>$this->Auth->user()['id'],'Dossiers.active'=>1,'Dossiers.archive'=>0])->contain(['Plans']);
        $plans = [];
    ...
    public function index()
    {
        $client_id = $this->Auth->user()['client_id'];
        $this->loadModel('Dossiers');
        $dossiers = $this->Dossiers->find()->where(['expert_id'=>$this->Auth->user()['id'],'Dossiers.active'=>1,'Dossiers.archive'=>0])->contain(['Plans']);
        $plans = [];
        foreach($dossiers as $dossier){
            if(!empty($dossier->plan))
                $plans[]=$dossier->plan->id;
