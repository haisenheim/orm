 ...
 * @var \App\Model\Entity\Mfinancement $mfinancement
 */
?>
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        <li class="heading"><?= __('Actions') ?></li>
        <li><?= $this->Html->link(__('Edit Mfinancement'), ['action' => 'edit', $mfinancement->id]) ?> </li>
        <li><?= $this->Form->postLink(__('Delete Mfinancement'), ['action' => 'delete', $mfinancement->id], ['confirm' => __('Are you sure you want to delete # {0}?', $mfinancement->id)]) ?> </li>
        <li><?= $this->Html->link(__('List Mfinancements'), ['action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Mfinancement'), ['action' => 'add']) ?> </li>
...
<div class="mfinancements view large-9 medium-8 columns content">
    <h3><?= h($mfinancement->name) ?></h3>
    <table class="vertical-table">
        ...
        <tr>
            ...
            <td><?= $this->Number->format($mfinancement->id) ?></td>
        </tr>
    </table>
    <div class="related">
        <h4><?= __('Related Dossiers') ?></h4>
        <?php if (!empty($mfinancement->dossiers)): ?>
        <table cellpadding="0" cellspacing="0">
            ...
            <?php foreach ($mfinancement->dossiers as $dossiers): ?>
            <tr>
                <td><?= h($dossiers->id) ?></td>
