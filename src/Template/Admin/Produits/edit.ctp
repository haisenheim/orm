...
<div class="content" style="font-family: 'Lato',sans-serif;">
    <h5 class="page-header text-center"><i class="fa fa-edit"></i> EDITION  PRODUIT : <span class="value"><?= $produit->name ?></span></h5>
    ...
    <div class="row">
        ...
        <div class="col-lg-3 col-md-4 col-sm-6">
            ...
            <input type="text" class="form-control" placeholder="ex: Cereales" name="name" id="name" value="<?= $produit->name ?>"/>
        </div>
        <div class="col-lg-2 col-md-3 col-sm-6 form-group">
            <label for="service">est un service? :</label>
            <input type="checkbox" checked="<?= $produit->service?true:false ?>" class="checkbox checkbox"  name="service" id="service"/>
        </div>
        ...
        <div class="col-lg-3 col-md-4 col-sm-12">
            ...
            <select name="sector_id" id="sector_id" class="form-control">
                <option value="<?= $produit->sector_id ?>"><?= $produit->sector->name ?></option>
    ...
    <table id="tab" class="table-condensed table-responsive">
        ...
        <?php foreach($produit->risques as $risque): ?>
            ...
            <tr data-id="<?= $risque->_joinData->id ?>">
                <td  data-risque_id="<?= $risque->id ?>"><?= $risque?$risque->name:'-' ?></td>
                <td contenteditable="true"><?= $risque->_joinData->name ?></td>
                <td contenteditable="true"><?= $risque->_joinData->causes ?></td>
                <td contenteditable="true"><?= $risque->_joinData->consequences ?></td>
                <td contenteditable="true"><?= $risque->_joinData->frequence ?></td>
                <td contenteditable="true"><?= $risque->_joinData->gravite ?></td>
...
<script>
    ...
    var saveUrl = "<?= $this->Url->build(['controller'=>'Produits', 'action'=>'editJson']) ?>";
