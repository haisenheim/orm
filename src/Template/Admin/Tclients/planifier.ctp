<h3 class="page-header text-center">EDITION DES OBJECTIFS DE CREATION DES COMPTES CLIENT</h3>
...
<script>
    var url = "<?= $this->Url->build(['controller'=>'Tclients','action'=>'perform']) ?>";
    $('#btn-save').click(function(){
        ...
        $.ajax({
            ...
            success:function(data){
                window.location.href="<?= $this->Url->build(['controller'=>'Tclients','action'=>'objectifs']) ?>"
            }
        });
